...
def draw_triangle(t, s, c):
    ...
    #store original color of turtle to restore at end
    OGcolor = t.color
    t.color = c
    for i in range(0,3):
        t.forward(s)
...
def draw_spiral(w, side, ang, n, sp):
    ...
    #clear window, create turtle, call helper, hide turtle
    w.clear()
    t = Turtle(w)
    #face south measured CCW from east
    t.heading = -90
...
def multi_polygons(w, side, k, n, sp):
    ...
    w.clear()
    t = Turtle(w)
    #face west measured CCW from east
    t.heading = 180
...
def multi_polygons_helper(t, side, k, n, sp):
    ...
    for i in range(0,k):
        ...
        #turns 360.0/k after each polygon
        t.right(306.0/k)
...
def radiate_petals(w, radius, width, n, sp):
    ...
    w.clear()
    t = Turtle(w)
    #turtle starts in default position, don't need to change heading
...
def radiate_petals_helper(t, radius, width, n, sp):
    ...
    OGheading = t.heading
    OGcolor = t.color
    if t.heading < 0.0 or t.heading > 360.0:
        t.heading - 360
    ...
    for i in range(0, n):
        ...
        t.left(360/n)
        #rotate then draw straight line
        t.forward(radius)
        t.backward(radius)
...
def tsquare(w, side, d, sp):
    ...
    assert is_number(side), report_error('invalid side value',side)

    w.clear()
    p = Pen(w,(0,0),'red','red',sp)
...
def tsquare_helper(p, x, y, side, d):
    ...
    assert is_valid_penmode(p), report_error('Invalid pen mode', p)
    assert is_valid_depth(d), report_error('invalid depth mode', d)
    assert is_number(x), report_error('invalid x number', x)
    assert is_number(y), report_error('invalid y number', y)
    assert is_number(side), report_error('invalid side number', side)
    ...
    if d == 0:
        ...
        fill_rect(p, x, y,side, side)
        newside = side/2
        tsquare_helper(p,x-newside,y-newside,newside,d-1)
        tsquare_helper(p,x+newside,y-newside,newside,d-1)
        tsquare_helper(p,x-newside,y+newside,newside,d-1)
        tsquare_helper(p,x+newside,y+newside,newside,d-1)
...
def hexaflake_helper(p, x, y, side, d):
    ...
    if d == 0:
        ...
        height = ((2*side)/3)*math.sin(math.pi/3)
        horiz = side * 2/3
        hexaflake_helper(p,x,y,side/3,d-1)
        hexaflake_helper(p, x + side/3, y + height, side/3, d-1)
        hexaflake_helper(p, x + side/3, y - height, side/3, d-1)
        hexaflake_helper(p, x - side/3, y + height, side/3, d-1)
        hexaflake_helper(p, x - side/3, y - height, side/3, d-1)
        hexaflake_helper(p, x + horiz, y, side/3, d-1)
        hexaflake_helper(p, x - horiz, y, side/3, d-1)
...
def kochflake(w, side, d, sp):
    ...
    assert is_number(side), report_error('invalid side number', side)
    assert is_valid_depth(d), report_error('invalid depth', d)
    assert is_valid_speed(sp), report_error('invalid turtle speed', sp)


    w.clear()
    loc = (-side/2, -(side/math.sqrt(3)))
    T = Turtle(w,loc,'blue',60,sp)
    T.visible = True
